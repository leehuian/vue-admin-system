var drawTopologyFunc = function (myVue, containerID) {
    ...
    self.unReachDataList = []; //未被访问到的funcid集合
    ...
    var TopoShape = {
        window_width: document.getElementById("container").offsetWidth, //内容窗口的宽度
        svg_width: document.getElementById("container").offsetWidth, //svg的宽度
        svg_height: 450,
    ...
    self.computedPoint = function () {
        var maxLenIndex = -1; //当天访问到的路径中，路径最长的下标
        var maxLen = -1; //当天访问到的路径中，路径的长度。
        var allMaxLenIndex = -1; //所有路径中，路径最大的下标
        var realMaxLen = -1; //计算路径最长的，该值和路径中是否包含未达节点无关。
        var unReachIndexList = []; //所有包含未达节点的路径的下标的集合，
        var reachIndexList = []; //已达节点的所有路径的下标的集合
        for (var i in self.sourceData) {
            var hasUnReachFlag = false; //预设置该路径不包含未达节点，然后再遍历，如果有未达节点，置为true；
            ...
            for (var j = 0; j < lineNodeArr.length; j++) {
                ...
                //首先判断是否已经得到包含未达节点的标志，如果确定了，就不进行后面的indexOf的操作
                if (!hasUnReachFlag && self.unReachDataList.indexOf(funcid) != -1) {
                    hasUnReachFlag = true;
                }
            }
            ...
            if (hasUnReachFlag) {
                //添加未达路径下标集合
                unReachIndexList.push(i);
            } else {
                //添加已达路径下标集合
                reachIndexList.push(i);
            }

            //计算最长的路径需要剔除包含未达节点的路径
            if (!hasUnReachFlag && lineNodeArr.length > maxLen) {
                maxLenIndex = i;
                maxLen = lineNodeArr.length;
            }

            if (lineNodeArr.length > realMaxLen) {
                realMaxLen = lineNodeArr.length;
                allMaxLenIndex = i;
            }
        }

        //svg左右两边各预留TopoShape.svg_padding的宽度，然后计算出两个节点之间的横向距离
        var _padding_width = (TopoShape.window_width - TopoShape.svg_padding * 2 - realMaxLen * TopoShape.node_width) / (realMaxLen - 1);
        ...
        if (TopoShape.node_padding_width < 100) {
            ...
            TopoShape.window_width = TopoShape.node_padding_width * (realMaxLen - 1) + realMaxLen * TopoShape.node_width + 2 * TopoShape.svg_padding;
            TopoShape.svg_width = TopoShape.window_width;
        }

        var _lujingLength = realMaxLen * (TopoShape.node_width + TopoShape.node_padding_width) - TopoShape.node_padding_width;
        var node_startX = TopoShape.window_width / 2 - _lujingLength / 2;

        //查询当天具有路径
        if (self.sourceData[maxLenIndex]) {
            //先计算不包含未达节点的路径最长的线路的位置，然后其他的线路根据这个最长的路径依次计算
            ...
            //当天不具有路径（也就是说在当天，从开始节点到结束节点，不存在访问）
            var lineNodeFuncidArr = self.sourceData[allMaxLenIndex].funcidPath.split("-");
            var levleTemp = 1; //等级临时变量，不使用路径下标的原因是防止路径上存在回路
            for (var i = 0; i < lineNodeFuncidArr.length; i++) {
                var funcid = lineNodeFuncidArr[i];
                if (i == lineNodeFuncidArr.length - 1) {
        ...
        //先计算已达路径，再计算未达路径
        var lujingIndexList = reachIndexList.concat(unReachIndexList);
        for (var lujingIndex in lujingIndexList) {
            var i = lujingIndexList[lujingIndex];
            var lineObj = self.sourceData[i];
            var lineNodeArr = lineObj.funcidPath.split("-");
    ...
    /**
     * 将所有未达节点的节点、连接线颜色进行修改。
     */
    self.drawUnReachColor = function () {
        //圆形节点颜色
        self.svg.selectAll("circle")
            .filter(function (d) {
                return self.unReachDataList.indexOf(d.funcid) > -1;
            })
            .attr('fill', "red");
        ...
        self.svg.selectAll('text.countText')
            .filter(function (d) {
                return self.unReachDataList.indexOf(d.begin.funcid) > -1 || self.unReachDataList.indexOf(d.end.funcid) > -1;
            })
            .text('');

        //连接线颜色
        var unLine = self.svg.selectAll('g')
            .filter(function (d) {
                return self.unReachDataList.indexOf(d.begin.funcid) > -1 || self.unReachDataList.indexOf(d.end.funcid) > -1
            });
        //曲线
     ...
     */
    self.create = function (sourceData, unReachDataList) {
        self.sourceData = sourceData; //传入需要处理的data
        self.unReachDataList = unReachDataList; //未被访问到的funcid的集合
        self.drawTopology();
    }
